require(['jquery', 'template', 'pager', 'tools'], function($, template) {
    var shopListMain = {
        ...
        queryParams: {
            ...
            theme: '',
            businessNo: '',
            armsRange: '', //穿越火线英雄武器数量
            accountType: '', //穿越火线账号类型
            ifPlayQualifying: '', //英雄联盟排位赛
            gradingId: '', //英雄联盟段位
            gradingFrameId: '', //英雄联盟段位框
            heroRange: '', //英雄联盟英雄区间
            skinRange: '', //英雄联盟皮肤区间
            ifPlayQualifying: '', //王者荣耀排位赛
            gradingId: '', //王者荣耀段位区间
            heroRange: '', //王者荣耀英雄数量
            skinRange: '', //王者荣耀皮肤数量
            gameId: 1109
        ...
        initEvents: function() {
            ...
            $('body').on('click', '.search-key span', function() {
                $('#seachinput').val($(this).text());
                _this._search($(this).text())
            });
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.code === "1") {
                        ...
                        $('title').text($('[data-gid="' + _this.queryParams.gameId + '"]').text() + '-' + $('title').text());
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.code === "1") {
                        ...
                        //_this.game_type = data.result.game_type;
                        if (data.result.game_type == 1) {
                            $('.g-phone-type').hide();
                        }
                        if (data.result.gameName != "穿越火线") {
                            $('.hero-number-weapons,.account-type').hide();
                        }
                        if (data.result.gameName != "英雄联盟") {
                            $('.hero-qualifier,.hero-grading,.hero-grading-box,.hero-number,.hero-skin-number').hide();
                        }
                        if (data.result.gameName != "王者荣耀") {
                            $('.honor-qualifier,.honor-grading,.honor-number,.honor-skin-number').hide();
                        }
                        // if (id) {
                        //     _this._renderServer(data.result);
                        // } else {
                        //     _this._renderArea(data.result);
                        // }
                    };
                }
            });
        ...
        _queryList: function() {
            ...
            $('.xb-pagination').setPager({
                ...
                ajax_function: function(pageindex) {
                    return $.ajax({
                        url: BASE_URL.goodsSolr + 'goods/findGoodsList',
                        // url: 'http://api.xubei.org/b/goods/findGoodsList',
                        //url: 'http://order-api.xubei.org:8081/goods-api/goods/findGoodsList',
